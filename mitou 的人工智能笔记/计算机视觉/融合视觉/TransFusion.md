...
- 只使用 LiDAR 时缺少语义信息，容易将大体形状相似的物体识别为 object 。
...
- 使用两个 Transformer decoder layer（attention 都不采用 mask，一次全部 parallel 计算）
...
针对 heatmap 的每个元素，选择 heatmap 中 top-$N$ candidates ，然后依据这些 candidates 的 grid cell 的位置，可以处理得到 object queries 。

query 的数量作为超参数（ top-$N$ ）。

在选择 top-$N$ 之前，To avoid spatially too closed queries ，选择 heatmap 水平切面的 local maximum 的位置用于组成 object queries ，它的值比它的八个邻居大或与之相等。

注意：

- 水平切面的 pooling，得到每个类别内的 local maximum，可以进一步对不同类别采用不同的 pooling size 。
- 若是采用每个类别的最大 score 的 local maximum，这相当于所有类别的 local maximum，会影响相靠近的类别的预测。

每个 query 包含：

- query position（维度为 $2$ ）

	localization of objects，BEV 的 grid cell 的中心的 2 维坐标。
...
heatmap 即是预测出了 BEV 的 grid cell 归属的类别，从而每个 query 的类别已知。（初步类别，decoder 预测的类别才是最终类别）
...
- 实际上，category-aware 也使得 object 能够与 background 在 decoder layer 中比对关联。
...
- query positions
  ...
  是使用 MLP 学习的 $d$ 维的位置编码（MLP 输入 query 的 grid cell 中心坐标，输出位置编码），然后 element-wise 加在 query features 上。

  - 原始的 Transformer decoder 只有一次 positional encoding。
  - TransFusion 进行了两次 position encoding 。
  - 作者没有尝试过 sin 和 cos positional encoding 。（[github issue](https://github.com/XuyangBai/TransFusion/issues/28)）
...
- LiDAR BEV features 和 Image Features 也要进行 positional encoding ，都采用 query position 的网格中心的方式。
...
对于 positional encoding ，TransFusion 与 DETR 的施加对象差异：

- TransFusion
	- Decoder
		- attention 的 Q，K，V
		- cross-attention 的 Q，K，V（论文和代码都是 Q，K，V，只是图中只画出了 Q）
- DETR
	- Encoder 的 Q，K
	- Decoder
		- attention 的 Q，K
		- cross-attention 的 Q，K
...
collapse 后的 image feature 使用 $1 \times W$ 的网格进行 positional encoding 。
...
- 2D backbone
  ...
  - Faster-RCNN 的 ResNet50 and FPN for Waymo。

  	自己用 Waymo 的 2D 标签训练 Faster-RCNN。

- 3D backbone

	SECOND（**？？？预训练后？？？**，训练时要更新参数）
...
2. 训练 fusion module + image-guided query initialization module（6 epochs）
...
不池化 pedestrian and traffic cone 的局部最大值 on nuScenes ，不池化 pedestrian and cyclist 的局部最大值 on Waymo 。（保持该 query 的）
...
- 第一行
	...
	检测出了远处的小目标（有个绿色的 GT 被楼房挡住了，没检测出来理所应当）
