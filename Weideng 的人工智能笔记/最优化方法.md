...
SGD + Momentum ，严格的说应该是 MBGD + Momentum 。
...
在有动量时，即使陷入局部最小值（梯度为 $0$），仍然可以依靠动量提供的方向继续运动：
...
一般的学习率调度是学习率越来越小，该策略比一般的学习率调度更鼓励进行探索，以跳出局部最小值，当已经收敛的一个较好的最优解时，学习率变大应当不易跳出该解。

#### SGDR

（SGD with Warm Restarts）

每周期学习率变小后不缓慢增加，直接调到最大，继续减小。
...
其中关于自适应学习率的部分 $\boldsymbol{v}_t = \beta_2 \boldsymbol{v}_{t-1} + (1 - \beta_2) \boldsymbol{g}_t^2$ ，其使用移动加权平均，当取默认超参数值 $\beta = 0.999$ 时，过去的梯度造成的影响将近持续 1000 次更新。此时大部分的梯度都会比较小，提供的方向信息微弱不显著，只有某几个 batch 的梯度会很大，提供明确的方向信息。当出现一下情况：
