...
RMSProp 是 AdaGrad 的改进，添加了超参数 $\alpha$ ，对梯度的大小做指数移动加权平均，然后开根号：
$$
...
$\alpha$ 也称为**衰减系数（Attenuation Coefficient）**指某些物理量例如光子、声波、电子、粒子的数量或能量等，在物体中单一方向行进贯穿的难易程度。
...
\begin{cases}
    ...
    \boldsymbol g^2_t = \boldsymbol g_t \odot \boldsymbol g_t
\end{cases}
$$

- $\boldsymbol m$ 是累计梯度的动量（含方向）。（控制方向）

- $\boldsymbol v$ 是累计梯度大小的变化剧烈程度（方差）。（控制速率，步长）
...
$$
\begin{cases}
\boldsymbol m_t = (1 - \beta_1) \sum\limits^t_{i = 1} \beta^{t - i}_1 \boldsymbol g_t \\\\
\boldsymbol v_t = (1 - \beta_2) \sum\limits^t_{i = 1} \beta^{t - i}_2 \boldsymbol g^2_t
\end{cases}
$$
其中 $\beta^t$ 代表 $\beta$ 的 $t$ 次方。

为了使 $\boldsymbol m_t$ 成为 $\boldsymbol g_t$ 的无偏估计，使 $\boldsymbol v_t$ 成为 $\boldsymbol g^2_t$ 的无偏估计，需要使 $E(\boldsymbol m_t) = E(\boldsymbol g_t)$ ，$E(\boldsymbol v_t) = E(\boldsymbol g^2_t)$ ，以后者为例：
$$
\begin{align}
E(\boldsymbol v_t) & = E[(1 - \beta_2) \sum\limits^t_{i = 1} \beta^{t - i}_2 \boldsymbol g^2_t] \\
...
较直观地说，由于 $\boldsymbol m, \boldsymbol v$  初始为 $\boldsymbol 0$ ，若不修正，则在默认初始值下初期 $\boldsymbol m_t, \boldsymbol v_t$ 的计算结果会偏小，从而初期步长 $\boldsymbol g\prime$ 过大。
...
$$
\boldsymbol \theta_t = \boldsymbol \theta_{t-1} - \alpha \boldsymbol g\prime\ , \ \ 
\boldsymbol g\prime = \frac {\boldsymbol {\hat m}_t} {\sqrt {\boldsymbol {\hat v}_t} + \epsilon}
$$

- $\boldsymbol g\prime$  是调整后的梯度。
- $\alpha$ 是调整后梯度的学习率。
- $\epsilon$ 是一个非常小的值，用于防止梯度方差为 $0$ 时导致的分母为 $0$ 。

### 效果

- Adam 使用了动量，具有动量的特性。
	- 提取了噪声中的有效信号，因为信号的总体方向在一定时间内是不会变的。
- Adam 使用了二阶矩估计（梯度的方差），用以衡量梯度变化的剧烈程度。
	- 抵消了一定的噪音，比如白噪音，它的均值为 $0$ ，多个白噪音叠加后会正负互相抵消。
	- 在梯度变化剧烈（噪音大，陡峭）的地方步长小，在变化平缓（噪音小，平坦）的地方步长大。
