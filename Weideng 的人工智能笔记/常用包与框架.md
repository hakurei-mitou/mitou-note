...
一般 Series 和 DataFrame 要先转化为 Ndarray 再转化为 Tensor，反之也要经过 Ndarray。
...
- Ndarray
	...
	多维数组类型，即嵌套数组，常简称 array 。NumPy 的数组只能包含相同类型的元素。
...
- Series
	...
	数据类型，相当于两列，有列标签，包含索引列和数据列，支持任何数据类型。
...
取行的某些列数据（逗号分隔，先行标签，后列标签，省略列标签则只取行）
df.loc[]
	只能使用标签索引。
...
```python
Variable.detach()
	将该结点的 Tensor 数据分离出来，同时阻断梯度的传播（只参与前向传播，不更新反向传播）。
Variable.grad
	获取该结点梯度信息。
Variable.grad_fn
...
class MyNN(nn.Module):   # 必须继承该类
    def __init__(self, input_dim):
        super(MyNN, self).__init__()   # 调用父类的初始化函数。
...
数据集需要预处理为简易的 “sample 列表" 形式的 Tensor，从而才能被更简易地划分，迭代。该类用于返回训练和测试（要编写分情况的代码）的一个 item 数据。

- random_split

	用于划分训练集和验证集，单位为 sample 。

- Dataset

  一个抽象类，用以使 Dataloader 区分 data 与 label 。

  - 所有自行构建的数据集类都必须继承它
  - 要重写 `__init__(), __getitem__(), __len__()` 方法。
...
- DataLoader
	...
	- 注意，test 时不要打乱顺序，要保证 `shuffle=False` 。
...
log_writer = SummaryWriter(log_dir="runs", flush_secs=120)   # 写入目录，写入时间间隔

for epoch in loader:
        log_writer.add_scalar(tag='loss_train', y, x)   # 图表的 tag 。
        log_writer.add_scalar('loss_valid', y, x)
