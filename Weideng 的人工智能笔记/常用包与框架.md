...
数据集需要预处理为简易的 “item" 的列表，从而才能被更简易地划分，迭代。
...
- DataLoader
	...
	可以将数据集转化为划分好  batch 的可迭代对象（data 的 loader），提供给训练器进行训练，每次迭代一个 batch ，batch 一般包含 data 与 label 两个部分。
...
尽量把准备工作使用代码实现，减少手动的操作，便于自己反复使用，以及其它平台和人员使用。
