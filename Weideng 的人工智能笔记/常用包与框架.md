...
- Series
	...
	数据类型，相当于一列，有列标签，包含索引列（即行标签）和数据列，支持任何数据类型。
...
取行的某些列数据（逗号分隔，先行标签，后列标签，省略列标签则取整行）
df.loc[]
	只能使用标签索引。
...
数据集需要预处理为简易的 “item 列表" 形式，从而才能被更简易地划分，迭代。
...
- Dataset
  ...
  - 所有自行构建的数据集类都必须继承它
  - 要重写 `__init__(self), __getitem__(self, idx), __len__()` 方法。
