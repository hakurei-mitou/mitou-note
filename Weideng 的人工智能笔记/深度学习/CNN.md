...
- 卷积使图像到特征图 channel 增多。
	...
	卷积后 channel 数目为卷积核数目和原 channel 数目的乘积。
...
- 填充（Padding）
  ...
  感受域完全保持在图像内的移动会导致过滤结果的边缘被裁剪，宽高变小。当想要保持宽高不变时，可以在图像边缘填充数值，一般补 $0$ 或直接使用边缘值。
...
- 注意
	...
	- filter 和 kernel 常互相指称，有些文献将二者等同，具体理解即可。
...
- 每个卷积层有多个神经元，它们共享权重（过滤器数据），输入感受域数据，监测感受域。
- filter 的数据，是需要用梯度下降求取的模型未知参数（神经元权重）。
...
卷积层的输出结果即是 feature map ，卷积层有多少个 filter 就会产生多少倍 channel 的 feature 。

feature map 相当于新产生的图像，每层 feature 相当于新图像的一个 channel 。

特征图之后即是通过激活函数，得到卷积层最终的输出。
...
卷积层的神经元共享权重参数，需要保持权重参数一致，对于反向传播到卷积层的各个神经元的梯度，可以采取均值，最大值或最小值等作为最终的共享梯度。
