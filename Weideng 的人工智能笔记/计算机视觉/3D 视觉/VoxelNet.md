...
An illustration is shown in Figure 2, where Voxel-1 has signifificantly more points than Voxel-2 and Voxel-4, while Voxel-3 contains no point.
...
**？？？往后堆叠的 VFE layer 对特征维度的处理如何？？？网上的代码似乎没有使用差值，等待 SECOND 的学习与确认**
...
每个非空的 voxel 都有了 voxel feature ，所有放在一起就是一个 sparse 4D tensor $C \times D' \times H' \times W'$ ，即使点云有 100k 的点，90% 以上的 voxel 也是空的，节约了内存和计算时间。
...
含有三个全卷积 block ，每个 block 的第一个 layer downsamples the feature map by half via a convolution with a stride size of 2 ，之后再跟几个 stride 为 1 的卷积 layer ，$\times q$ 代表使用了几个 layer（原文为 filter ，一个卷积 layer 有一个 filter，多个 kernel）。
...
一个 3D GT 为：$(x_c^g, y_c^g, z_c^g, l^g, w^g, h^g, \theta^g)$ ，其中，$x_c^g, y_c^g, z_c^g$ 是 center location ，$l^g, w^g, h^g$ 是 length，width，height ，$\theta^g$ 是 yaw rotation around Z-axis 。
...
作者初始化一个 $K \times T \times 7$ 的 tensor 作为 voxel input feature buffer ，其中，$K$ 是非空 voxel 的最大数量，$Y$ 是 voxel 的最大点数，$7$ 是每个点的 encoding 维度。

points 在处理之前是随机的。

对 point cloud 的每个点，检查：

- 如果相应的 voxel 已经初始化，并且少于 $T$  个点，则插入点到该 voxel ，否则忽略该点。
...
最后，从 voxel coordinate buffer 中取出 voxel 的坐标，对应到 voxel-wise feature ，组成 sparse tensor ，即形成  dense voxel grid ，可用于后续的 convolutional middle layer 和 RPN 。
...
对 GT 内的点整体移动或旋转，移动后进行碰撞检测，删除掉和其它 GT 混在一起的不可能在现实中出现的情况。
