...
默认清理下不会记录**管理语句**和**不使用索引的查询语句**，可以如下修改配置文件：
...
- 垂直拆分
	...
	- 垂直分表
		...
		以字段为依据，将不同字段分布到不同表中。（一般各自通过主键或外键相关联）

- 水平拆分

	- 水平分库

		以字段为依据，将不同位置的记录分布到不同的数据库中。
...
- Mycat 配合数据库本身的复制功能，可以解决读写分离的问题，但是针对分表分库的问题，不是完美的解决。或者说，至今为止，业界没有完美的解决方案。
	...
	- 不能完美解决主要是**联表查询的问题**
		...
		Mycat 支持两个表联表的查询，**多余两个表的查询不支持**。 其实，很多数据库中间件关于分表分库后查询的问题，都是需要自己实现的，而且基本都不支持联表查询，Mycat 已经算做地非常先进了。分表分库之后联表查询问题，可以通过合理数据库设计来避免。
